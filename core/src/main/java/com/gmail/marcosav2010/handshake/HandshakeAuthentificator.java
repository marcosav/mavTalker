...
public class HandshakeAuthentificator {
    ...
    private final Map<String, ConnectionToken> localStorage = new ConcurrentHashMap<>();
    private final Map<String, ConnectionToken> localTempStorage = new ConcurrentHashMap<>();
    private final Map<InetSocketAddress, ConnectionToken> remoteStorage = new ConcurrentHashMap<>();
    ...
    private String generateAddressKey(byte[] requesterConnectionKeyBytes, ConnectionToken ct, byte mark)
        ...
        return Utils.encode(markedBytes);
    }
    ...
    public ConnectionToken parseAddressKey(byte[] remoteAddressKeyBytes, byte type)
        ...
        switch (type) {
            case PRIVATE_KEY -> connectionKey = getConnectionKey();
            case PUBLIC_KEY -> {
                connectionKey = PUBLIC_CONNECTION_KEY;
                isPublic = true;
            }
            default -> throw new IllegalArgumentException("Invalid address key type.");
        }
